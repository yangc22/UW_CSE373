...
public class PacketSorter {
    ...
    public static void mergeSort(Packet[] array, Comparator<Packet> comparator) {
    	Packet[] newArray = new Packet[array.length];
        if (array.length != 0) {
     ...
     * 				the packets to sort
     * @param newArray
     * 				the auxiliary array used to store the sorted packets
     * @param startIndex
     * 				start index for the array to be sorted
     * @param endIndex
     * 				end index for the array to be sorted
     * @param comparator
     * 				comparator used to compare the order of the given packet
     */
    private static void mergeSort(Packet[] array, Packet[] newArray, int startIndex,
    		int endIndex, Comparator<Packet> comparator) {
     ...
     * 				comparator used to compare the order of the given packet
     */
    private static void merge(Packet[] array, Packet[] newArray,
    		int startIndex, int midIndex, int endIndex,  Comparator<Packet> comparator) {
    		int i = startIndex;
    		int j = midIndex + 1;
    ...
    /**
     * finds the index of the minimum packet from the unsorted part
     * @param array
     ...
     */
    private static int findMin(Packet[] array, Comparator<Packet> comparator, int i) {
    	Packet min = array[i];
    	int position = i;
    ...
    /**
     * swap the packet at the current position with the minimum packet in the unsorted part
     * @param array
     * 				the array of packet that will be sorted
     ...
     */
    private static void swap(Packet[] array, int unsortedPosition, int sortedPosition) {
    	Packet temp = array[unsortedPosition];
